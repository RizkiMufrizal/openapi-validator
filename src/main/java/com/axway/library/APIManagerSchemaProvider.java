...
public class APIManagerSchemaProvider {
    ...
    public String getSchema(String apiId) throws Exception {
        Utils.traceMessage("Getschema debug seb", TraceLevel.DEBUG);
        if (!useOriginalAPISpec) {

            Utils.traceMessage("call getFrontendAPISpec debug seb", TraceLevel.DEBUG);
            return getFrontendAPISpec(apiId);
        } else {
            Utils.traceMessage("call getBackendAPIID debug seb", TraceLevel.DEBUG);
    ...
    private String getFrontendAPISpec(String apiId) {
        Utils.traceMessage("getFrontendAPISpec debug seb", TraceLevel.DEBUG);
        Utils.traceMessage("getFrontendAPISpec debug seb inside try", TraceLevel.DEBUG);
        ...
        Utils.traceMessage("getFrontendAPISpec debug seb response from 2.0 call" + httpResponse.getBody() + " code : " + statusCode, TraceLevel.DEBUG);
        if (statusCode != 200) {
            Utils.traceMessage("getFrontendAPISpec debug seb not 200", TraceLevel.ERROR);
            if (statusCode == 500) {
                Utils.traceMessage("getFrontendAPISpec debug seb code = 500", TraceLevel.ERROR);
                if (httpResponse.getBody() != null) {
                    Utils.traceMessage("getFrontendAPISpec debug seb response != null", TraceLevel.ERROR);
                    if (httpResponse.getBody().contains("Failed to download API")) {
                        Utils.traceMessage("getFrontendAPISpec debug seb response.contains", TraceLevel.ERROR);
                        Utils.traceMessage("No SwaggerVersion 2.0 API-Specification found, trying to download OpenAPI 3.0 specification.", TraceLevel.ERROR);
    ...
    private String getBackendAPISpec(String backendApiId) throws Exception {
        ...
        Utils.traceMessage("getBackendAPISpec debug seb", TraceLevel.DEBUG);
        int statusCode = httpResponse.getHttpCode();
        if (statusCode != 200) {
    ...
    private String getBackendAPIID(String apiId) throws Exception {
        Utils.traceMessage("getBackendAPIID debug seb", TraceLevel.DEBUG);
        ...
        if (statusCode != 200) {
            if (statusCode == 403 && httpResponse.getBody() != null && httpResponse.getBody().contains("Forbidden")) {
                Utils.traceMessage("Please check the given API-ID: '" + apiId + "' is correct, as the API-Manager REST-API returns forbidden for an incorrect API-ID.", TraceLevel.ERROR);
            }
